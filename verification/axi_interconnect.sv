module axi_interconnect (
    ...
    always_comb begin
        if (slave_awaddr[31:16] == 16'h4000)      wr_sel = SEL_PWM;
        else if (slave_awaddr[31:16] == 16'h4001) wr_sel = SEL_TIMER;
        else if (slave_awaddr[31:16] == 16'h4002) wr_sel = SEL_GPIO;
        else if (slave_awaddr[31:16] == 16'h4003) wr_sel = SEL_UART;
        else                                       wr_sel = SEL_NONE;
    end
    always_comb begin
        if (slave_araddr[31:16] == 16'h4000)      rd_sel = SEL_PWM;
        else if (slave_araddr[31:16] == 16'h4001) rd_sel = SEL_TIMER;
        else if (slave_araddr[31:16] == 16'h4002) rd_sel = SEL_GPIO;
        else if (slave_araddr[31:16] == 16'h4003) rd_sel = SEL_UART;
        else                                       rd_sel = SEL_NONE;
    end
    ...
    assign pwm_awvalid   = slave_awvalid & (wr_sel == SEL_PWM);
    assign timer_awvalid = slave_awvalid & (wr_sel == SEL_TIMER);
    assign gpio_awvalid  = slave_awvalid & (wr_sel == SEL_GPIO);
    assign uart_awvalid  = slave_awvalid & (wr_sel == SEL_UART);
    ...
    assign pwm_arvalid   = slave_arvalid & (rd_sel == SEL_PWM);
    assign timer_arvalid = slave_arvalid & (rd_sel == SEL_TIMER);
    assign gpio_arvalid  = slave_arvalid & (rd_sel == SEL_GPIO);
    assign uart_arvalid  = slave_arvalid & (rd_sel == SEL_UART);
